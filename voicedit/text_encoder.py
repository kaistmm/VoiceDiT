...
class TextEncoder(nn.Module):
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels, 
                 ...
                 p_dropout, window_size=None, spk_emb_dim=64, cond_spk=False):
        ...
        self.encoder = Encoder(n_channels + (spk_emb_dim if cond_spk else 0), filter_channels, n_heads, n_layers, 
                               kernel_size, p_dropout, window_size=window_size)

        self.proj_m = torch.nn.Conv1d(n_channels + (spk_emb_dim if cond_spk else 0), n_feats, 1)
        self.proj_w = DurationPredictor(n_channels + (spk_emb_dim if cond_spk else 0), filter_channels_dp, 
