...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if isinstance(model_output, dict) and 'dur_loss' in model_output:
                terms['dur_loss'] = model_output['dur_loss']
            if isinstance(model_output, dict) and 'prior_loss' in model_output:
                terms['prior_loss'] = model_output['prior_loss']
        else:
