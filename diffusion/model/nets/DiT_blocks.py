...
class MultiHeadCrossAttention(nn.Module):
    def __init__(self, d_model, num_heads, cross_attn_dim=None, attn_drop=0., proj_drop=0., **block_kwargs):
        ...
        if cross_attn_dim is None:
            cross_attn_dim = d_model
        self.cross_attn_dim = cross_attn_dim

        self.q_linear = nn.Linear(d_model, d_model)
        self.kv_linear = nn.Linear(cross_attn_dim, d_model*2)
...
class CaptionEmbedder2(nn.Module):
    """
    Embeds class labels into vector representations. Also handles label dropout for classifier-free guidance.
    """
...
class CaptionEmbedder(nn.Module):
    ...
    def __init__(self, in_channels, hidden_size, uncond_prob, act_layer=nn.GELU(approximate='tanh')):
        super().__init__()
        self.y_proj = Mlp(in_features=in_channels, hidden_features=hidden_size, out_features=hidden_size, act_layer=act_layer, drop=0)
    ...
    def token_drop(self, caption, force_drop_ids=None):
        ...
        if force_drop_ids is None:
            ...
            drop_ids = force_drop_ids == 1
        null_y = self.y_embedding.repeat(caption.shape[1],1)
        caption = torch.where(drop_ids[:, None, None, None], null_y, caption)
        return caption
