...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, cross_attn_dim=None, mlp_ratio=4.0, drop_path=0., window_size=0, input_size=None, use_rel_pos=False, **block_kwargs):
        super().__init__()
        ...
        self.cross_attn = MultiHeadCrossAttention(hidden_size, num_heads, cross_attn_dim, **block_kwargs)
...
class DiT(nn.Module):
    ...
    def forward(self, x, timestep, y, mask=None, class_labels=None, data_info=None, **kwargs):
        """
        Forward pass of DiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        y: (N, 1, T, C) tensor of caption labels
    ...
    def forward_with_dpmsolver(self, x, timestep, y, mask=None, class_labels=None, **kwargs):
        """
    ...
    def forward_with_cfg(self, x, timestep, y, cfg_scale, mask=None, class_labels=None, guidance="single", **kwargs):
        ...
        model_out = self.forward(combined, timestep, y, mask, class_labels, kwargs)
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], self.x_embedder.grid_size, lewei_scale=self.lewei_scale)
...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0, lewei_scale=1.0):
    """
    ...
    grid_h = np.arange(grid_size[0], dtype=np.float32) / lewei_scale
    grid_w = np.arange(grid_size[1], dtype=np.float32) / lewei_scale
...
if __name__ == '__main__':
    ...
    model = DiT_cross_models['DiT_L/2'](
        input_size = (256, 16),
        in_channels = 8,
    ...
    x = torch.randn(1, 8, 256, 16).to(device)
    timestep = torch.Tensor([999]).to(device)
    y = torch.randn(1, 1, 1024, 64).to(device)
    mask = torch.ones(1, 1, 1, 1024).to(device)
    class_labels = torch.randn(1, 512).to(device)

    macs, params = profile(model, inputs=(x, timestep, y, mask, class_labels))
    flops = macs * 2
